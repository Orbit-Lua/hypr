...
- `lua/config/autostart.toml` is limited to compositor-owned runtime work. It
  currently starts the rainbow border helper; long-running session apps belong
  to the workstation Nix configuration repository (`~/.config/nix`).
...
- Graphical-session user units: the workstation Nix configuration at
  `~/.config/nix/modules/home/graphical-session.nix`
- Graphical-session architecture and ownership policy:
  `~/.config/nix/docs/desktop-session.md`
...
- Do not add long-running session applications to Hyprland autostart. Keep them
  in systemd user units or system XDG autostart according to their existing
  owner.
...
- Long-running graphical-session applications are managed by the workstation
  Nix configuration (`~/.config/nix`), not this Hyprland repository.
- Do not duplicate Vicinae, hyprpolkitagent, Noctalia, Quickshell Overview,
  tray applets, Vesktop, or system XDG autostart entries in
...
- Re-check the workstation Nix configuration
  (`~/.config/nix/modules/home/graphical-session.nix` and
  `~/.config/nix/docs/desktop-session.md`) before documenting
  graphical-session service ownership or enablement.
